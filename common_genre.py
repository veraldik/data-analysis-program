...
with open('finalnotfixed.csv') as f:
    ...
    for row in reader:
        ...
        if frequency == '0 - 30 minutes':
            list1.append(int(num_of_genres))
        if frequency == '30 - 60 minutes':
            list2.append(int(num_of_genres))
        if frequency == '1 - 2 hours':
            list3.append(int(num_of_genres))
        if frequency == '2 - 4 hours':
            list4.append(int(num_of_genres))
        if frequency == '5 or more hours':
            list5.append(int(num_of_genres))
